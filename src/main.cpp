...
#if defined(NDEBUG)
# error "Skicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc59174c22a2c2a296a1addce6229fecb0d1c15f437a16fe2fe144e119dbcec84");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Skicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Skicoin Signed Message:\n";
...
{
    // Skicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Skicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 24 * 60 * 60; // Skicoin: 5 days
static const int64 nTargetSpacing = 5 * 60; // Skicoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Skicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Skicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc28fad4bec326d1755702714579361d85e2d468c7285d6fc10310afc29842021");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "SZ Times 10/Apr/2020 Die EU-Staaten werden sich dramatisch verschulden müssen";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04bcb8bdb3efebeada20c6fd21faabed359e52ba65c80a938f7d16582cfe3e639dee3fcef138644a27b424c26dab60b0ced4cfaefc9c46e8723bb299052fe55200") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1586517327;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084740319;

        if (fTestNet)
        {
            block.nTime    = 1586517303;
            block.nNonce   = 386223998;
        }
      ...
      //// debug print
        ...
        assert(block.hashMerkleRoot == uint256("0x3bbaf1129aa48a8823cc5ace1e11a90ddc721ea91abe0b08e8325eeb9b576b8e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc9, 0xb3, 0xdl }; // Skicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SkicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SkicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SkicoinMiner(CWallet *pwallet)
{
    printf("SkicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("skicoin-miner");
    ...
    try { loop {
        ...
        printf("Running SkicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SkicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SkicoinMiner, pwallet));
}
